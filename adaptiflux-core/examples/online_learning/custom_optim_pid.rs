...
//!
//! Build with:
//!   cargo run --example online_learning/custom_optim_pid --features custom_optim
...
use adaptiflux_core::{CoreScheduler, LocalBus, OnlineAdaptationEngine, OnlineAdaptationHook, ResourceManager, RuleEngine, Zoooid, ZoooidId};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let pid_agent = Zoooid::new(
        pid_id,
        Box::new(adaptiflux_core::agent::blueprint::pidzooid::PIDzooidBlueprint {
    ...
    for i in 0..20 {
        ...
        if i % 5 == 0 {
            let kp = scheduler
                .agents
                .get(&pid_id)
                .and_then(|h| h.state.downcast_ref::<adaptiflux_core::agent::blueprint::pidzooid::PIDzooidState>())
