...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("🚀 Starting swarm search (running for 100 iterations)...\n");

    // Run the scheduler for fixed iterations
    if let Err(e) = scheduler.run_for_iterations(100).await {
        eprintln!("❌ Scheduler error: {}", e);
    }

    // Wait for stats
    if tokio::time::timeout(Duration::from_secs(1), stats_handle)
        .await
        .is_err()
    {
        println!("\n📊 Statistics collection timeout");
    }
